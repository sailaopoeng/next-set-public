    ...
    <div className="space-y-3">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold capitalize text-slate-700">
            {session.status}
          </span>
        </div>
        {session.notes ? (
      ...
      {blocks.map((block) => block.groupId ? (
        <section className="rounded-2xl border border-emerald-300 bg-white p-4 shadow-sm" key={block.id}>
          ...
          <div className="mt-3 space-y-2">
            {block.items[0].session_sets.map((_, roundIndex) => (
              <div className="rounded-xl bg-emerald-50/60 p-2.5" key={roundIndex}>
                <p className="mb-1 text-[11px] font-extrabold uppercase tracking-wide text-emerald-700">Round {roundIndex + 1}</p>
